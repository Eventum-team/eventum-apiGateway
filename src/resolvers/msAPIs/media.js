...
const deleteUserProfile = async ({ id_type, _id }) => {
    try {
      const { data } = await axios.delete(`${completeProfileURI}/delete/${id_type}/${_id}`);
      return data;
    } catch (error) {
      console.log(error);
    }
...
const deleteGroup = async ({ id_group, _id }) => {
    try {
      const { data } = await axios.delete(`${completeGroupURI}/delete/${id_group}/${_id}`);
      return data;
    } catch (error) {
      console.log(error);
    }
...
const deleteGroupProfile = async ({ id_group, _id }) => {
    try {
      const { data } = await axios.delete(`${completeGroupURI}/delete/profile/${id_group}/${_id}`);
      return data;
    } catch (error) {
      console.log(error);
    }
